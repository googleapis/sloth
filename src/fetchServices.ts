...
export async function listServices(client: any, serviceNames: string[], nextPageToken: string = ''): Promise<any> {
    ...
    try {
        ...
        if ('authentication' in serviceConfig) {
            if ('rules' in serviceConfig['authentication']) {
                const scopes: boolean[] = serviceConfig.authentication.rules.map((item: any) => {
                    if ('oauth' in item) {
...
export async function exportApisToSheets() {
    ...
    const values: string[][] = await Promise.all(services.map(async (s) => {
